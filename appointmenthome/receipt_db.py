...
def find_today_duplicate(patient, total, prefix=None):
    """ตรวจว่าคนไข้คนนี้ ยอดเท่ากัน (และโหมด/รหัสเดียวกัน) ออกใบเสร็จไปแล้ว 'วันนี้' หรือยัง
       คืนเลขใบเสร็จเดิมถ้าเจอ (เช่น 'CA690615-01'), คืน None ถ้ายังไม่มี
       ใช้เตือน/บันทึกซ้ำกันการออกใบซ้ำโดยไม่ตั้งใจ"""
    ...
    try:
        if prefix:
            row = conn.execute(
                ...
                "WHERE patient=? AND ABS(total-?)<0.01 AND prefix=? AND receipt_no LIKE ? "
                "ORDER BY issued_at DESC LIMIT 1",
                (patient, float(total or 0), prefix, f"%{date_key}%"),
            ).fetchone()
        else:
            row = conn.execute(
                "SELECT receipt_no FROM receipts "
