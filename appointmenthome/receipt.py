...
# ==========================================
# 3. 📊 ฟังก์ชันสร้างไฟล์ Excel ใบเสร็จฉบับปรับปรุง (หน้าตาเหมือน HTML 100%)
# ==========================================
...
def generate_receipt_excel_file(data):
    ...
    # ปรับความกว้างคอลัมน์ให้สมดุลเหมือนหน้ากระดาษ
    ws.column_dimensions['A'].width = 15
    ws.column_dimensions['B'].width = 45
    ws.column_dimensions['C'].width = 25

    # --- 1. ใส่โลโก้ (ดึงจาก Base64 ที่มีอยู่แล้ว) ---
    try:
        img_data = base64.b64decode(logo_base64)
        img = ExcelImage(BytesIO(img_data))
        img.width = 75   # ปรับขนาดความกว้างโลโก้
        img.height = 75  # ปรับขนาดความสูงโลโก้
        ws.add_image(img, 'A1')
    except Exception as e:
        pass # หากไม่มีไลบรารีจัดการภาพ จะข้ามการใส่โลโก้ไปก่อน
    ...
    ws.merge_cells(f'A{current_row}:B{current_row}') # Merge ให้ข้อความภาษาไทยมีพื้นที่
    ...
    current_row += 1
    ws[f'C{current_row}'] = f"{data['receiver']}"
    ws[f'C{current_row}'].font = Font(name='Tahoma', size=11, bold=True)
    ws[f'C{current_row}'].alignment = Alignment(horizontal='center')
    
    current_row += 1
    ws[f'C{current_row}'] = "ผู้รับเงิน"
    ws[f'C{current_row}'].font = Font(name='Tahoma', size=11)
    ws[f'C{current_row}'].alignment = Alignment(horizontal='center')

    current_row += 3
    
    # --- 8. ข้อมูลท้ายใบเสร็จ (Footer 2 ฝั่ง แบบเดียวกับ HTML) ---
    # บรรทัด 1
    ws[f'A{current_row}'] = "โรงพยาบาลโฮม ฉะเชิงเทรา"
    ws[f'A{current_row}'].font = Font(name='Tahoma', size=10, bold=True, color="555555")
    ws[f'C{current_row}'] = "บริษัท สื่อ การแพทย์ จำกัด"
    ws[f'C{current_row}'].font = Font(name='Tahoma', size=10, bold=True, color="555555")
    ws[f'C{current_row}'].alignment = Alignment(horizontal='right')
    current_row += 1

    # บรรทัด 2
    ws[f'A{current_row}'] = "149/1 ถ.ฉะเชิงเทรา-บางปะกง ต.หน้าเมือง อ.เมือง จ.ฉะเชิงเทรา 24000"
    ws[f'A{current_row}'].font = Font(name='Tahoma', size=10, color="555555")
    ws[f'C{current_row}'] = "149/1 ถ.ฉะเชิงเทรา-บางปะกง ต.หน้าเมือง"
    ws[f'C{current_row}'].font = Font(name='Tahoma', size=10, color="555555")
    ws[f'C{current_row}'].alignment = Alignment(horizontal='right')
    current_row += 1

    # บรรทัด 3
    ws[f'A{current_row}'] = "Tel. 038-511-123 | E-mail: Hospitalashome@gmail.com"
    ws[f'A{current_row}'].font = Font(name='Tahoma', size=10, color="555555")
    ws[f'C{current_row}'] = "อ.เมือง จ.ฉะเชิงเทรา 24000"
    ws[f'C{current_row}'].font = Font(name='Tahoma', size=10, color="555555")
    ws[f'C{current_row}'].alignment = Alignment(horizontal='right')
    current_row += 1
    
    # บรรทัด 4
    ws[f'C{current_row}'] = "เลขประจำตัวผู้เสียภาษีอากร 0245563001367"
    ws[f'C{current_row}'].font = Font(name='Tahoma', size=10, color="555555")
    ws[f'C{current_row}'].alignment = Alignment(horizontal='right')
...
# เช็คสถานะว่าเคยออกใบเสร็จไปแล้วหรือยัง (ถ้าออกแล้วจะเป็น True)
is_already_issued = st.session_state.get("issued_no") is not None

with col_btn1:
    # เพิ่มเงื่อนไข disabled=is_already_issued เข้าไปในปุ่ม
    if st.button("🧾 ออกเลขใบเสร็จ + บันทึกลงระบบ", type="primary", width='stretch', disabled=is_already_issued):
        if not ex_data["name"]:
    ...
    # แจ้งเตือนผู้ใช้หากปุ่มถูกล็อกไว้
    if is_already_issued:
        st.warning("⚠️ เอกสารนี้ถูกออกใบเสร็จและบันทึกไปแล้ว เพื่อป้องกันความผิดพลาดระบบจึงล็อกการกดซ้ำ หากต้องการทำรายการใหม่กรุณากด 'เริ่มใบเสร็จใหม่' หรืออัปโหลดไฟล์ใหม่ครับ")
