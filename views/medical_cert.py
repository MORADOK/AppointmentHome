...
def html_driving(d):
    ...
    """
    return _wrap_html("ใบรับรองแพทย์ (สำหรับใบอนุญาตขับรถ)", body)
...
cert_type = st.selectbox(
    ...
    ["ใบรับรองแพทย์ทั่วไป", "ใบรับรองแพทย์ 5 โรค (ก.พ./สมัครงาน)", "ใบรับรองแพทย์ (ใบอนุญาตขับขี่)"],
)
...
if cert_type == "ใบรับรองแพทย์ทั่วไป":
    ...
    # แบบ 5 โรค และ ขับขี่ ใช้ฟิลด์เดียวกันส่วนใหญ่
    today_thai = format_thai_date(datetime.today())
    ...
    if cert_type == "ใบรับรองแพทย์ (ใบอนุญาตขับขี่)":
        e1, e2 = st.columns(2)
        with e1:
